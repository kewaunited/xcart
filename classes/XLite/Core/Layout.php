...
class Layout extends \XLite\Base\Singleton
{
     ...
     * @param \XLite\View\AView $viewer    View object
     *
     * @return array
     */
    public function getTemplateFullPath($shortPath, $viewer)
    {
        ...
        if (1 === count($parts)) {
            $skin = $viewer->currentSkin ?: $currentSkin;
            $locale = $viewer->currentLocale ?: $currentLocale;

            $result = ('parent' === $shortPath)
                ? $this->getResourceParentFullPath($currentTemplate, $skin, $locale)
            ...
            $result = $this->getResourceParentFullPath($parts[1], $skin, $locale);
    ...
    {
        $found = false;
        $paths = $this->getSkinPaths($this->currentInterface, $this->getResourceModulePath($shortPath));
        reset($paths);
     ...
     * @return string
     */
    public function getResourceSkinFullPath($shortPath, $skin, $interface = null)
    {
        $result = null;
    ...
    {
        return $this->currentInterface . '.' . $this->mailInterface . '.' . $interface . '.' . $shortPath;
    }
     ...
     * @return array
     */
    public function getSkinPaths($interface = null, $resourceModulePath = '', $reset = false)
    {
        $interface = $interface ?: $this->currentInterface;

        $key = \XLite::MAIL_INTERFACE === $interface
            ? $interface . '-' . $this->getMailInterface()
            : $interface;
        ...
        if ($reset || !isset($this->skinPaths[$key])) {
            ...
            $commonInterface = \XLite::MAIL_INTERFACE === $interface
                ? $this->getMailInterface()
                : null;

            foreach ($this->getSkins($interface) as $skin) {
                foreach ($locales as $locale) {
                    $localeFsPath = $locale ? LC_DS . $locale : '';
                    $localeWebPath = $locale ? '/' . $locale : '';

                    if ($commonInterface) {
                        $mailCommonFsPath = LC_DS . 'mail';
                        $mailCommonWebPath = '/' . 'mail';

                        if ($isModuleResource) {
                            $mailCommonFsPath
                                = LC_DS . str_replace('/', LC_DS, $resourceModulePath) . $mailCommonFsPath;
                            $mailCommonWebPath = '/' . $resourceModulePath . $mailCommonWebPath;
                        }

                        foreach ($this->getSkins($commonInterface) as $commonSkin) {
                            $this->skinPaths[$key][] = array(
                                'name' => $commonSkin,
                                'fs' => LC_DIR_SKINS . $commonSkin . $localeFsPath . $mailCommonFsPath,
                                'web' => static::PATH_SKIN . '/' . $commonSkin . $localeWebPath . $mailCommonWebPath,
                                'locale' => $locale,
                            );
                        }
                    }
