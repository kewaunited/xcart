...
class USPS extends \XLite\Model\Shipping\Processor\AProcessor
{
    ...
    public function getRates($inputData, $ignoreCache = false)
    {
        ...
        } elseif (\XLite\Module\CDev\USPS\Main::isStrictMode()) {
...
OUT;
                    ...
                    if ($postage) {
                        foreach ($postage as $k => $v) {
                            ...
                            foreach ($rateTypes as $rt) {
                                ...
                                if (0 < doubleval($rateValue)) {
                                    $ratePrices[$rt] = doubleval($rateValue);
                                }
                            }

                            // Postage data
                            $postageData = array(
                                'CLASSID' => 'D-' . $xml->getArrayByPath($v, '@/CLASSID') . '-' . $this->getUniqueMethodID($serviceName),
    ...
    protected function getRatePrice($ratePrices)
    {
        ...
        $useRateType = \XLite\Core\Config::getInstance()->CDev->USPS->use_rate_type;
...
OUT;
            ...
            if ($postage) {
                foreach ($postage as $k => $v) {
                    ...
                    $result['postage'][] = array(
                        'CLASSID' => 'I-' . $xml->getArrayByPath($v, '@/ID') . '-' . $this->getUniqueMethodID($serviceName),
                        'MailService' => $this->getUSPSNamePrefix() . $serviceName,
    ...
    {
        return \XLite\Core\Config::getInstance()->CDev->USPS->use_cod_price
    ...
    protected function addShippingMethod($postage)
    {
        ...
        if (!in_array($postage['CLASSID'], $this->newMethods)) {
            ...
            $lastMethod = \XLite\Core\Database::getRepo('XLite\Model\Shipping\Method')
                ->findOneMaxPositionByProcessor($this->getProcessorId());
