...
class ApiRequest extends \XLite\Base
{
    ...
    /**
     * X-Payments Connector config data
     */
    ...
    public function send($target, $action, array $data = array(), $apiVersion = false)
    {
        ...
        if (!$apiVersion) {
            ...
            $apiVersion = $this->config->xpc_api_version;
        }
    ...
    public function encryptXml($data)
    {
        ...
        $key = openssl_pkey_get_public($this->config->xpc_public_key);
        if (!$key) {
    ...
    protected function sendApiRequest($request)
    {
        // HTTPS request
        $post = array(
            'cart_id' => $this->config->xpc_shopping_cart_id,
            'request' => $request,
        );
        ...
        curl_setopt($ch, CURLOPT_URL, $this->config->xpc_xpayments_url . '/api.php');
    ...
    protected function decryptXml($data)
    {
        ...
        // Decrypt
        $res = openssl_get_privatekey($this->config->xpc_private_key, $this->config->xpc_private_key_password);
