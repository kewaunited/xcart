...
class FEDEX extends \XLite\Model\Shipping\Processor\AProcessor
{
    ...
    protected function doRequest($data, $ignoreCache)
    {
        ...
        if ($availableMethods) {
            ...
            try {
                ...
                if ($ignoreCache === true) {
                    $xmlDataLog = preg_replace(
                        '|<v13:AccountNumber>.+</v13:AccountNumber>|i',
                        '<v13:AccountNumber>xxx</v13:AccountNumber>',
                        $xmlData
                    );

                    $xmlDataLog = preg_replace(
                        '|<v13:MeterNumber>.+</v13:MeterNumber>|i',
                        '<v13:MeterNumber>xxx</v13:MeterNumber>',
                        $xmlDataLog
                    );
                    ...
                    $xmlDataLog = preg_replace(
                        '|<v13:Password>.+</v13:Password>|i',
                        '<v13:Password>xxx</v13:Password>',
                        $xmlDataLog
                    );
    ...
    protected function getXMLData($data)
    {
        ...
        $fedexOptions['ship_date_ready']
            = date('c', \XLite\Core\Converter::time() + intval($fedexOptions['ship_date']) * 24 * 3600);
        ...
        foreach (array('fdxe', 'fdxg', 'fxsp') as $code) {
            if (isset($fedexOptions[$code]) && 'Y' == $fedexOptions[$code]) {
                $carrierCodes
                    .= str_repeat(' ', 9) . '<v13:CarrierCodes>' . strtoupper($code) . '</v13:CarrierCodes>' . PHP_EOL;
            }
        }
...
OUT;
        ...
        if (
            'Y' == $fedexOptions['opt_saturday_pickup']
            && 6 == date('w', \XLite\Core\Converter::time() + intval($fedexOptions['ship_date']) * 24 * 3600)
        ) {
...
OUT;
    ...
    protected function getRateAmount($entry)
    {
        ...
        $rateCurrency
            = $xml->getArrayByPath($entry, 'RatedShipmentDetails/ShipmentRateDetail/TotalNetCharge/Currency/0/#');
        ...
        if (empty($estimatedRate)) {
            $estimatedRate
                = $xml->getArrayByPath($entry, 'RatedShipmentDetails/ShipmentRateDetail/TotalNetCharge/Amount/0/#');
        }
