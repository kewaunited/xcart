...
abstract class ARepo extends \Doctrine\ORM\EntityRepository
{
    ...
    public function find($id)
    {
        ...
        return null !== $id ? parent::find($id) : null;
    }
     ...
     * @return boolean
     */
    protected function isIndexExists($tableName, $indexName)
    {
        $indexesOfTable = \XLite\Core\Database::getEM()->getConnection()->getSchemaManager()
            ->listTableIndexes($tableName);
        return array_reduce(
            $indexesOfTable,
            function ($carry, $index) use ($indexName){
                return $carry ?: $indexName == $index->getName();
            },
            false
        );
    }
    ...
    /**
     * Get not existed indexes list
     * 
     * @param array  $schema Schema
     * 
     * @return array Schema
     */
    protected function getNotExistedIndexes(array $schema)
    {
        $pattern = '/DROP INDEX `?IDX_(.*?)`? ON `?([a-zA-Z0-9-_]+)`?/Ss';
        return array_reduce(
            $schema,
            function ($carry, $schemaLine) use ($pattern) {
                if ( preg_match($pattern, $schemaLine, $matches) ) {
                    $indexName = $matches[1];
                    $tableName = $matches[2];

                    $indexExists = $this->isIndexExists($tableName, 'IDX_' . $indexName);
    ...
    public function processSchema(array $schema, $type)
    {
        if (\XLite\Core\Database::SCHEMA_UPDATE === $type || \XLite\Core\Database::SCHEMA_CREATE === $type) {
            ...
            list($enabledTables, ) = \XLite\Core\Database::getInstance()
            ...
            if ($disabledTables) {
                ...
                preg_match_all(
                    '/ALTER TABLE `?([a-zA-Z0-9-_]+)`? DROP FOREIGN KEY `?([a-zA-Z0-9-_]+)`?/S',
                    implode(';', $schema),
                    $droppedKeys
                );
                ...
                if (!empty($droppedKeys[0]) && is_array($droppedKeys[0])) {
                    ...
                    for ($i = 0; $i < count($droppedKeys[0]); $i++) {
                        $foreignKeys = \XLite\Core\Database::getEM()->getConnection()->getSchemaManager()
                            ->listTableForeignKeys($droppedKeys[1][$i]);
                    ...
                    if ($filterOnDropKeys) {
                        ...
                        $indexNames = preg_filter(
                            '/DROP INDEX `?IDX_(.*?)`? ON `?[a-zA-Z0-9-_]+`?/Ss',
                            '\1',
                            $schema
                        );
                        ...
                        // See BUG-1963
                        $filterOnDropKeysForIndexes = array_filter(
                            $indexNames,
                            function ($idxName) use ($filterOnDropKeys) {
                                return array_reduce(
                                    $filterOnDropKeys,
                                    function ($carry, $fkName) use ($idxName) {
                        ...
                        $schema = preg_grep(
                            '/DROP INDEX `?IDX_(?:' . implode('|', $filterOnDropKeysForIndexes) . ')`? ON `?[a-zA-Z0-9-_]+`?/Ss',
                            $schema,
                            PREG_GREP_INVERT
                        );
                    }
            ...
            $schema = preg_grep(
                '/DROP INDEX `?IDX_(?:' . implode('|', $notExistedIndexes) . ')`? ON `?[a-zA-Z0-9-_]+`?/Ss',
                $schema,
                PREG_GREP_INVERT
            );
