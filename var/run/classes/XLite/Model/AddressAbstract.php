 ...
 * @MappedSuperClass (repositoryClass="\XLite\Model\Repo\Address")
 */
abstract class AddressAbstract extends \XLite\Model\Base\PersonalAddress
{
    ...
    public function getCountry()
    {
        ...
        if (!$result) {
            ...
            if ($countryField) {
                $result = \XLite\Core\Database::getRepo('XLite\Model\Country')->findOneByCode(\XLite\Core\Config::getInstance()->Shipping->anonymous_country);
            }
        }
    ...
    {
        if ($this->getState() && $this->getCustomState() !== $this->getStateName()) {
