 ...
 */
abstract class PaymentActionsUnit extends \XLite\View\Order\Details\Admin\PaymentActionsUnitAbstract implements \XLite\Base\IDecorator
{
    /**
    ...
    public function isDisplayAmount()
    {
        ...
        if (
            ...
            if (
                \XLite\Model\Payment\BackendTransaction::TRAN_TYPE_ACCEPT == $type
                || \XLite\Model\Payment\BackendTransaction::TRAN_TYPE_DECLINE == $type
            ) {
                $result = false;
            }
        }
    ...
    {
        $location = '\'' . $this->getActionURL() . '&amount=' . '\' + $(\'#' . $this->getAmountFieldId() . '\').val()';

        $code = 'self.location=' . $location . ';';
