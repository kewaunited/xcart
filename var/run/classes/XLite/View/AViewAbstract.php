...
abstract class AViewAbstract extends \XLite\Core\Handler
{
    ...
    protected function getTemplateFile($template = null)
    {
        ...
        return \XLite\Singletons::$handler->layout->getTemplateFullPath($template, $this);
    }
     ...
     * @return \XLite\View\AView
     */
    protected function getChildWidget($class = null, array $params = array())
    {
    ...
    protected function getSVGImage($path, $interface = null)
    {
        ...
        $path = \XLite\Singletons::$handler->layout->getResourceFullPath($path, $interface);
    ...
    {
        return $this->isCacheAllowed('') && \XLite\Core\WidgetCache::getInstance()->has($this->getCacheParameters());
    }
    ...
    public function buildURL($target = '', $action = '', array $params = array(), $forceCuFlag = null)
    {
        return \XLite\Core\Converter::makeURLValid(
            \XLite\Core\Handler::buildURL($target, $action, $params, null, false, $forceCuFlag)
        );
    }
