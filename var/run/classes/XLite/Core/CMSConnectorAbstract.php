 ...
 * CMS connector
 */
abstract class CMSConnectorAbstract extends \XLite\Base\Singleton
{
    /**
    ...
    protected $widgetsList = array(
        ...
        '\XLite\View\ItemsList\Product\Customer\Category\Main' => 'Category products list',
    ...
    {
        if (isset($name)) {
            $result = isset($this->widgetsList[$name]) ? $this->widgetsList[$name] : null;

        } else {
            $result = $this->widgetsList;
        }
    ...
    public function getPageInstance($target, array $params = array())
    {
        ...
        return new \XLite\Core\WidgetDataTransport(
            $class ? new $class(array('target' => $target) + $params) : null
        );
    }
